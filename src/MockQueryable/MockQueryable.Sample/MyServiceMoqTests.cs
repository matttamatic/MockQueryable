...
namespace MockQueryable.Sample
{
    ...
    public class MyServiceMoqTests
    {
            ...
            var users = new List<UserEntity>
      {
        ...
        [TestCase]
        public async Task DbSetFindAsyncUserEntityWithEFLike()
        {
            //arrange
            var userId = Guid.NewGuid();
            var users = new List<UserEntity>
      {
        ...
        new UserEntity
        {
          ...
          DateOfBirth = DateTime.Parse("01/20/2018", UsCultureInfo.DateTimeFormat)
            ...
            //act
            var result = await (from u in ((DbSet<UserEntity>)userRepository.GetQueryable())
                                where EF.Functions.Like(u.FirstName, "%Name3")
                                select u).FirstAsync();
